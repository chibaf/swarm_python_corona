...
#--------------------関数の宣言---------------------------------
def make_pos_FA(pos,goal): #ゴールから最遠のヒツジを計算
    D_sg=np.linalg.norm(pos - goal, axis=1) #距離の計算
    x_j_FA=pos[np.argmax(D_sg)] #距離の最大値を持つヒツジ
    return x_j_FA

def make_vd1(pos_xd, x_j_FA): #牧羊犬の分離力を計算
    posVN=np.linalg.norm(pos_xd - x_j_FA, ord=2) #ノルムを計算
    return -(pos_xd - x_j_FA)/posVN #反発力を計算する． 

def make_vd2(pos_xd, x_j_FA): #牧羊犬の結合力を計算
    posVN=np.linalg.norm(pos_xd - x_j_FA, ord=2) #ノルムを計算
    return (pos_xd - x_j_FA)/posVN**3
...
for i in range(0,iter):
    ...
    pos +=v #ヒツジの位置の更新
    x_j_FA=make_pos_FA(pos, goal) #最遠のヒツジの座標を計算
    #牧羊犬の制御処理
    vd1=make_vd1(pos_xd, x_j_FA) #分離力の計算
    vd2=make_vd2(pos_xd, x_j_FA) #結合力の計算
    vd3=power5.make_vd4(pos_xd, goal) #ゴールとの分離力の計算
    #牧羊犬の制御処理
...
ani.save("FAT.gif", writer="Pillow") #gif画像としてアニメを保存