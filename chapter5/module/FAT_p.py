...
def make_vd1(pos_xd,x_j_FA):#牧羊犬の反発力を計算するモジュール
    ...
    return -posVD #反発力を計算する． 

def make_vd2(pos_xd,x_j_FA): #牧羊犬の引力を計算するモジュール
    posV= pos_xd - x_j_FA #重心位置の差分を計算
    posVN=np.linalg.norm(posV,ord=2) #ノルムを計算
    #pos_sub_norm3=np.where(pos_sub_norm3 < 1,1,pos_sub_norm3) #ノルムの下限値を演算
...
def make_vd3(pos_xd,goal): #牧羊犬と目的地に対する斥力を計算するモジュール
